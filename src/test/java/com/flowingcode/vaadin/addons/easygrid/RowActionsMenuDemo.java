...
/**
 * Demo view showing row actions as a dropdown menu.
 *
 * @author Javier Godoy / Flowing Code
 */
@DemoSource
@PageTitle("Actions as Menu")
@SuppressWarnings("serial")
@Route(value = "easy-grid/row-actions-menu", layout = EasyGridDemoView.class)
public class RowActionsMenuDemo extends Div {
  ...
  public RowActionsMenuDemo() {
    ...
    // Menu item enabled only when a phone number is available
    grid.addRowAction("Call", VaadinIcon.PHONE.create(), person ->
        Notification.show("Calling " + person.getPhoneNumber()))
        .enabledWhen(person -> person.getPhoneNumber() != null);

    // Shown only for active persons
    grid.addRowAction("Deactivate", VaadinIcon.CLOSE_CIRCLE.create(), person ->
        Notification.show("Deactivated: " + person.getFirstName()))
        .visibleWhen(Person::isActive);

    // Shown only for inactive persons
    grid.addRowAction("Activate", VaadinIcon.CHECK_CIRCLE.create(), person ->
        Notification.show("Activated: " + person.getFirstName()))
        .visibleWhen(person -> !person.isActive());

    // Present the row actions as an overflow (⋮) menu
