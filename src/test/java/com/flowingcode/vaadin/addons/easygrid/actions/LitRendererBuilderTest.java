...
public class LitRendererBuilderTest {
  ...
  @Test
  public void spread_emitsAttrAndPropMaps() {
    LitRendererBuilder<Object> b = newBuilder();
    b.tag("x", () -> b.spreadAllAttributesAndProperties(item -> (HasElement) () -> new Element("test")));
    assertEquals(
        guarded("<x .attr=${item.actions[0]} .prop=${item.actions[1]}></x>"),
        b.getTemplate());
  }

  @Test
  public void spread_build_attrMapHoldsAttributes_propMapHoldsProperties() {
    LitRendererBuilder<Pojo> b = newPojoBuilder();
    ...
    assertEquals("vaadin:check", attrMap.getString("icon"));
    assertEquals("val", attrMap.getString("extra"));
    JsonObject propMap = actions.getObject("1");
    assertEquals("done", propMap.getString("symbol"));
  }
